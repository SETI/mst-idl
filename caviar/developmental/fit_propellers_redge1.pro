...
if keyword_set(dolzr) and not keyword_set(impact) then niceplot = 1
...
for j=j1,j2 do begin
  ...
  fittype = 1
  if not keyword_exists(bke) then bke = 0
  noplot = 1
  @run_edgefit
  noplot = 0
...
;  wdelete, 8
  ...
  if keyword_set(impact) then begin
    ...
    if (where( ledgebnd ne [0,0] ))[0] eq -1 then $
                                     ledgebnd = [min(lonx),max(lonx)]
    good0a = where( redge[0,*] ge ledgebnd[0,j] and $
                    redge[0,*] le ledgebnd[1,j], ng0a )
    ;ng0a = n_elements(ng0a)
    xr1 = redge[ 0, good0a[[0,ng0a-1]] ]
  endif else begin
    if keyword_set(niceplot) then begin
    ...
    endelse 
  endelse
  yr1 = [ min(redge[1,good0a]), max(redge[1,good0a]) ]
  yr1 = yr1 + [-0.1,0.1]*(yr1[1]-yr1[0])
  if keyword_set(impact) then ytit='Radius (km)' else begin
    ytit='              Radius (km)'
  endelse
  plot_nosci, xr1, yr1, /nodata, /xs, /ys, ytit=ytit, xtickn=notn
...
;  endif 
  yr2 = [ min(redge[2,good0a]), max(redge[2,good0a]) ]
  ...
  if keyword_set(edgebnd[1,j]) and not keyword_set(redo) then begin
    for k=0,nedgebnd-1 do if !d.name ne 'PS' then begin
      wset, 9
  ...
  if keyword_set(impact) then begin
    ...
    if !d.name eq 'PS' then begin
      ...
      l = 0
    endelse 
    oplot, ledgebnd[*,j], co=clr, l=1, /noclip, $
