FUNCTION phasearray_u, filename, rarray=rarray, larray=larray, rs=rs, $
	xarray=xarray, yarray=yarray
	 image_name=filename
	noplot=1
	 @caviar_u
	 
	cspice_spkez,-32L,et,'J2000','NONE',799L,state,ltime
	camera_coord = rotate(state[0:2],1)
	
	cspice_spkez,10L,et,'J2000','NONE',799L,state,ltime
	...
	aimpoint=[i*rs,j*rs]
	p2radec_u,pixscale,cmat,nl,aimpoint[1],aimpoint[0],aimp_ra,aimp_dec
	p2ralon_u,cmat,et,polera,poledec,sc,aimp_ra,aimp_dec,aimp_radius,aimp_lon
	get_ring,et,aimp_radius,0,0,polera,poledec,1,0,799L,-32L,lons=aimp_lon,j2000_xyz=aimpoint_coord
