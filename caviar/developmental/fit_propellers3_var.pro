...
for j=j1,j2 do begin
  ...
  window, 10, xs=sz0[1]+25, ys=sz0[2]
  plotalso = -1
  ...
  if plotalso ne -1 then begin
    ...
    window, 12, xs=sz1[1]+25, ys=sz1[2], xpos=sz0[1]*1.5
...
;             /device, color=ctpurple()
      plots, lindgen((2*p1[2]+1)>1)*2 + p1[4]-p1[2]*2, p1[5] + p1[3]*2* $
             sqrt( 1 - (( lindgen((2*p1[2]+1)>1) - p1[2] )/p1[2])^2 ), $
             /device, thick=2
;      plots, lindgen((2*p1[2])>1) + p1[4]-p1[2], p1[5] - p1[3]* $
;             sqrt( 1 - (( lindgen((2*p1[2])>1) - p1[2] )/p1[2])^2 ), $
;             /device, color=ctpurple()
