...
pro radial_filter, rbksub_im,  _rawim, radi, val, et, polera, poledec, cam_params, nl, $
       ...
       primary=primary, median=median, blank_cray=blank_cray, $
       input_val=input_val, val_im=val_im, fadefac=fadefac
...
if keyword_set(input_val) then begin
  if not keyword_exists(val) then input_val = 0 else begin
    if n_elements(val) ne n_elements(radi) then input_val = 0
  endelse 
endif
if not keyword_set(input_val) then begin
  radi = ( radbins[1:nbins] + radbins[0:nbins-1] ) / 2
  val = fltarr(nbins)
  radscan_np = fltarr(nbins)
  errbar = fltarr(nbins)
endif else nbins = n_elements(radi)
...
;see the radial background subtraction section below to see how polyflux works
  ...
  if not keyword_set(input_val) then begin
    val[i] = polyflux( polygon/_nl, rawim, _nl, _nl, zoomfactor, $
                       errbar=_errbar, radscan_np=_np, mask=mask, $
                       median=median )
    radscan_np[i] = _np
    errbar[i] = _errbar
  endif
...
val_im = rbksub_im
if not keyword_exists(fadefac) then _fadefac = 1 else _fadefac = fadefac
rbksub_im = _rawim - rbksub_im*_fadefac

; If both _cray and blank_cray are set, then set cray locations to zero
if keyword_set(_cray) and keyword_set(blank_cray) then begin
  rbksub_im[_cray[*,1],_cray[*,0]] = 0
endif

;clean up
foo = where( val eq -1, count )
if count gt 0 and not keyword_set(input_val) then begin
