...
if keyword_set(trialmode) then _ring_rads_keep[*] = 1
if not keyword_set(ring_rads_maxlam) then ring_rads_maxlam = 0
if not keyword_set(ring_rads_keep) then ring_rads_keep = 0
if not keyword_set(ring_rads_sigma_manual) then ring_rads_sigma_manual = 0
if keyword_set(usecurrentrings) then begin &$
  ...
  if count eq 0 then begin &$
    ...
    if keyword_set(_ring_rads_maxlam) then begin &$
      ring_rads_maxlam = _ring_rads_maxlam[foo] &$
    endif &$
    if keyword_set(_ring_rads_keep) then begin &$
      ring_rads_keep = _ring_rads_keep[foo] &$
    endif &$
    if keyword_set(_ring_rads_sigma_manual) then begin &$
      ring_rads_sigma_manual = _ring_rads_sigma_manual[foo] &$
    endif &$
  endelse &$
  ...
  if keyword_set(trimrr) then begin &$
    ...
    if count gt 0 then begin &$
      ...
      if keyword_set(_ring_rads_maxlam) then begin &$
        ring_rads_maxlam = ring_rads_maxlam[foo2] &$
      endif &$
      if keyword_set(_ring_rads_keep) then begin &$
        ring_rads_keep = ring_rads_keep[foo2] &$
      endif &$
      if keyword_set(_ring_rads_sigma_manual) then begin &$
        ring_rads_sigma_manual = ring_rads_sigma_manual[foo2] &$
      endif &$
    endif &$
  endif &$
  ...
  if keyword_set(specifyrings) then begin &$
    ...
    if keyword_set(_ring_rads_maxlam) then begin &$
      ring_rads_maxlam = _ring_rads_maxlam[specifyrings] &$
    endif &$
    if keyword_set(_ring_rads_keep) then begin &$
      ring_rads_keep = _ring_rads_keep[specifyrings] &$
    endif &$
    if keyword_set(_ring_rads_sigma_manual) then begin &$
      ring_rads_sigma_manual = _ring_rads_sigma_manual[specifyrings] &$
    endif &$
  endif &$
