...
pro p2ralon_quicker_u,cmat,et,polera,poledec,sc,RA,dec,radius,lon,$
...
for i=0,1 do begin
  ...
  if i eq 0 and n_elements(et) eq 1 then begin
    ;cspice_spkez,799L,et-mean(light_time),'J2000','CN+S',sc,state,ltime
    cspice_spkez,799L,et-mean(light_time),'J2000','NONE',sc,state,ltime
    xyz_J2000 = rebin( reform(-state[0:2],1,3), nn, 3 )
  endif else for j=0l,nn-1 do if done[j] ne 1 then begin
    ;cspice_spkez,799L,et-light_time[j],'J2000','CN+S',sc,state,ltime
    if n_elements(et) eq 1 then begin
      cspice_spkez,799L,et-light_time[j],'J2000','NONE',sc,state,ltime
    endif else begin
      cspice_spkez,799L,et[j]-light_time[j],'J2000','NONE',sc,state,ltime
    endelse
