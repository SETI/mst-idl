...
pro caviar_zoom,im,rawim,cam_params,cmat,nl,et,sc,polera,poledec, $
                coords,planet_coords,ring_coords,stars,planet,s_names, $
		plan_names,$
...
1:	if 1 eq 1 then begin	
	...
	xyouts,20,430,'Radius = '+strcompress(string(radius))+'  '+$
	      'Longitude = '+strcompress(string(lon),/remove_all),/device


	xyouts,250,70,'line = '+strcompress(string(line))+'  '+$
	      'sample = '+strcompress(string(sample))+'  DN = '$
               +strcompress(string(long(rawim[x,y]))),/device
	xyouts,250,55,'RA = '+strcompress(string(RA))+'  '+$
	      'dec = '+strcompress(string(dec),/remove_all),/device
	xyouts,250,40,'Radius = '+strcompress(string(radius))+'  '+$
	      'Longitude = '+strcompress(string(lon),/remove_all),/device
