slash = strpos( /reverse_search, image_name, '/' )
...
help,not keyword_set(_rawim) and not keyword_set(usehistogram)
...
if (not keyword_set(_rawim) and not keyword_set(nohistogram)) or (keyword_set(usehistogram) and not keyword_set(manualstretch)) then begin &$
