...
dtwistarray=make_array(317,/float)
dlarray=make_array(317,/float)
drarray=make_array(317,/float)
delr=make_array(317,/float)
minl=224.18415
mod_dr=make_array(317,/float)
tbar=make_array(317, /float)
meanl=make_array(317,/float)
...
if keyword_set(redgearr2) || keyword_set(redgearr3) then begin
	...
	if keyword_set(redgearr2) then begin
		oplot,redgearr2[rind,*],redgearr2[rind+1,*],color=clr(0)
                outermean=133743.84 ;this outermean business plots the mean radius
                   outermean=redgearr2[rind+1,*]*0+outermean
                   oplot,xr,outermean,color=clr(2)
	endif

	if keyword_set(redgearr3) then begin
		oplot,redgearr3[rind,*]+dt3,redgearr3[rind+1,*],color=clr(1)
                outermean=133743.84 ;plots the mean radius
                   outermean=redgearr3[rind+1,*]*0+outermean
                   oplot,xr, outermean,color=clr(2)
	endif
endif
...
if keyword_set(redgearr0) || keyword_set(redgearr1) then begin
	...
	if keyword_set(redgearr0) then begin
		oplot,redgearr0[rind,*],redgearr0[rind+1,*],color=clr(0)
                  innermean=133422.25 ;plots the mean radius
                    innermean=redgearr0[rind+1,*]*0+innermean
                      oplot,xr, innermean,color=clr(2)
        endif

	if keyword_set(redgearr1) then begin
		oplot,redgearr1[rind,*]+dt1,redgearr1[rind+1,*],color=clr(1)
                  innermean=133422.25 ;plots the mean radius
                    innermean=redgearr1[rind+1,*]*0+innermean
                      oplot,xr, innermean,color=clr(2)
	endif
endif
...
while reply1 eq '' do begin
  ...
  read, reply1
;if imagenum lt 189 then reply1='a' else read, reply1 ;316
  case reply1 of
