...
pro display_reproj1, screenx, screeny, _rpj_sz, _rpi, draw, drawid, noplot=noplot, reproj_zoom=reproj_zoom
    ...
    while nowin eq 0 do begin &$
      ...
      winset, 31+jw, nowin &$
    endwhile
    if keyword_set(reproj_zoom) then begin
      rpi = rebin( _rpi, _rpj_sz[1]*reproj_zoom, _rpj_sz[2]*reproj_zoom )
      rpj_sz = size(rpi)
      rpjz = ' (Zoom: '+strtrim(rpj_sz[1],2)+'x'+strtrim(rpj_sz[2],2)+')'
    endif else begin
      rpi = _rpi
      rpj_sz = _rpj_sz
      rpjz = ''
    endelse
    base = widget_base(title='IDL '+strtrim(31+jw,2)+':  Reprojected Image '+strtrim(_rpj_sz[1],2)+'x'+strtrim(_rpj_sz[2],2))
