pro radial_scan, _rawim, radi, val, et, polera, poledec, cam_params, nl, $
       ...
       primary=primary, noclip=noclip, nbins=_nbins, quiet=quiet, debug=debug
...
if keyword_set(phi) then begin
  ...
  if keyword_set(_nbins) then nbins = _nbins else begin
...
if not keyword_set(noscan) then for i=0,nbins-1 do begin
  if i mod fix(float(nbins)/20) eq 0 then begin
    ...
    endif else if not keyword_set(quiet) then print, i, ' / ', nbins
  endif
  if keyword_set(phi) then begin
  ...
  goodx = where( polygon[*,0] ge 0 and polygon[*,0] le _nl, countx )
  goody = where( polygon[*,1] ge 0 and polygon[*,1] le _nl, county )
  if countx gt 0 and county gt 0 then begin
...
if count gt 0 and not keyword_set(noclip) then begin
