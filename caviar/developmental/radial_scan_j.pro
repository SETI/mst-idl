pro radial_scan_j, _rawim, radi, val, et, polera, poledec, pixscale, nl, $
...
if not keyword_set(primary) then primary=599L
if keyword_set(vertical) then begin
  ...
  get_ring,et,mnrad,mnlon,mxlon,polera,poledec,nnn,radscan_region1,primary,-98L,state=state
  get_ring,et,mxrad,mnlon,mxlon,polera,poledec,nnn,radscan_region2,primary,-98L,state=state
endelse
image_coords_j,radscan_region1,cmat,vobs_planet,pixscale,nl,radscan_region1_coords
image_coords_j,radscan_region2,cmat,vobs_planet,pixscale,nl,radscan_region2_coords
...
if keyword_set(_cray) then begin
  p2radec_quicker_j,pixscale,cmat,nl,_cray[*,0],_cray[*,1],cray_RA,cray_dec
  p2ralon_quicker_j,cmat,et,polera,poledec,-82L,cray_RA,cray_dec,crayr,crayl
...
if not keyword_set(noscan) then for i=0,nbins-1 do begin
  ...
  if keyword_set(phi) then begin
    ...
    if keyword_set(vertical) then begin
      ...
      get_ring,et,radbins[i+1],mnlon,mxlon,polera,poledec,nnn,outer,primary,-98L,state=state
    endelse
    image_coords_j,outer,cmat,vobs_planet,pixscale,nl,outer_coords
