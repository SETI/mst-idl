...
function edgefit, rpi, rrpi, mnrad, mxrad, mnlon, mxlon, $
                  ...
                  _redge_sigma_linsam=redge_sigma_linsam, debug=debug
...
for i=bke[0]+1,bke[1]-1 do begin
  if keyword_set(smoothdata) then begin
    ...
    data = smooth(rrpi[i,*],10,/edge_truncate)
  endif else begin
    data = rrpi[i,*]
  endelse
...
;    while data[yyi-1] gt data[peak]/2 do yyi = yyi - 1
    ...
    if keyword_set(nopad) then begin
      ...
      ;pad = data[yi]<data[yo]
      pad = median([ data[yi:yi+9], data[yo-9:yo] ])
  ...
  if keyword_set(debug) then begin
    if debug gt 0 then stop else if debug eq -i then stop
  endif
end
catch, /cancel
