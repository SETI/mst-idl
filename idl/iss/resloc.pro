...
function resloc_rres_root_bw, r
  common resloc1, sourcegm, omegasat, patternspeed, mm, inner
  return, sqrt(caviar_omega2(lc82=lc82,r)) - inner*sqrt(caviar_nu2(lc82=lc82,r))/mm - patternspeed
end
...
; Change 21 May 2018:  Fixed some wrong formulas.  M.S. Tiscareno
function resloc, rl1, rl2, satnum, res_descrip=res_descrip, short=short, $
	omegasat=omegasat, kappasat=kappasat, nusat=nusat, rsat=rsat, $
        ...
        omegares=omegares, pp=_pp, kk=_kk, mm=mm, $
...
if not keyword_set(mm) then mm = 0
if keyword_set(_kk) then kk = _kk else kk = 0
if keyword_set(_pp) then pp = _pp else pp = 0

if keyword_set(mm) or keyword_set(kk) or keyword_set(pp) then begin
  stop  ; Need to figure out what to do if reosnances are specified in this way.
endif

if keyword_set(bending) or keyword_set(vertical) then begin
  if keyword_set(cer) or keyword_set(cir) then stop, 'Conflicting resonance types'
  restype = 'vertical'
endif else if keyword_set(cer) then begin
  if keyword_set(cir) then stop, 'Conflicting resonance types'
  restype = 'cer'
endif else if keyword_set(cir) then begin
  restype = 'cir'
endif else restype = 'Lindblad'

inner = -1
if rl1 gt rl2 then inner = 1
if rl1 eq -1 and rl2 eq 0 then inner = 1
if rl1 eq rl2 then stop, 'Invalid resonance label'
...
case restype of
  ...
  'vertical': begin
    ...
    pp = inner
    kk = rl1 - mm - pp
    if kk*inner lt 0 then stop, 'No first-order vertical resonances'
  end
  'cer': begin
    mm = rl2
    kk = rl1 - mm
  ...
  'cir': begin
    ...
    pp = rl1 - mm
    kk = 0
    if pp mod 2 ne 0 then stop, 'No odd-ordered CIRs'
  end
  else:  stop, 'Invalid resonance type'
endcase
mm = double(mm)
kk = double(kk)
pp = double(pp)
...
esat = _esat[_satnum]
isat = _isat[_satnum]
res_descrip = satnames[_satnum] + ' ' + strtrim(fix(rl1),2) + ':' + strtrim(fix(rl2),2)
...
if abs(rl1) eq 1 and mm eq 1 and inner eq 1 then begin
  ...
  ;rres0 = ( gm / omegasat^2 / rl1^2 * rl2^2 ) ^ (1./3) + [-5,0,5]
  rres0 = caviar_omega_to_r( omegasat * rl1 / rl2 ) + [-5,0,5]
endelse
