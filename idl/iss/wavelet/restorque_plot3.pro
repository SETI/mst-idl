...
if keyword_set(cycle) then begin
  if not keyword_exists(suff) then suff = '_a1' else case suff of
    '_a1': suff = '_a2'
    '_a2': suff = '_a3'
    '_a3': suff = '_a4'
    '_a4': suff = '_a5'
...
if keyword_set(findfile(savefile1)) then restore, savefile1 else begin
  ...
  foo = where( strmid(_ring_rads_legend,0,6) eq 'Ja 7:4' )
  _ring_rads_index_outcome[foo] = 2
;  foo = where( strmid(_ring_rads_legend,0,6) eq 'Ep 7:4' )
;  _ring_rads_index_outcome[foo] = 2
  ...
  _ring_rads_index_outcome[foo] = 5
  foo = where( _ring_rads_legend eq 'Ia -1:0 BW' )
  _ring_rads_index_outcome[foo] = 1
  foo = where( strmid(_ring_rads_legend,0,9) eq 'Ja 7:4 BW' )
  _ring_rads_index_outcome[foo] = 2
  foo = where( strmid(_ring_rads_legend,0,9) eq 'Ep 7:4 BW' )
  _ring_rads_index_outcome[foo] = 2
  foo = where( strmid(_ring_rads_legend,0,10) eq 'Ja 11:8 BW' )
  _ring_rads_index_outcome[foo] = 2
  foo = where( strmid(_ring_rads_legend,0,10) eq 'Ep 11:8 BW' )
  _ring_rads_index_outcome[foo] = 2
  foo = where( strmid(_ring_rads_legend,0,10) eq 'Ja 12:9 BW' )
  _ring_rads_index_outcome[foo] = 2
  foo = where( strmid(_ring_rads_legend,0,10) eq 'Ep 12:9 BW' )
  _ring_rads_index_outcome[foo] = 2
  foo = where( strmid(_ring_rads_legend,0,11) eq 'Ja 14:11 BW' )
  _ring_rads_index_outcome[foo] = 2
  foo = where( strmid(_ring_rads_legend,0,11) eq 'Ep 14:11 BW' )
  _ring_rads_index_outcome[foo] = 2
  foo = where( strmid(_ring_rads_legend,0,11) eq 'Ja 15:12 BW' )
  _ring_rads_index_outcome[foo] = 2
  foo = where( strmid(_ring_rads_legend,0,11) eq 'Ep 15:12 BW' )
  _ring_rads_index_outcome[foo] = 2
  foo = where( strmid(_ring_rads_legend,0,11) eq 'Ja 16:13 BW' )
  _ring_rads_index_outcome[foo] = 2
  foo = where( strmid(_ring_rads_legend,0,11) eq 'Ep 16:13 BW' )
  _ring_rads_index_outcome[foo] = 2

  save, _ring_rads, _ring_rads_legend, _ring_rads_index_outcome, dirs, ndirs, $
        nrr, ring_rads_index, filename=savefile1
...
if keyword_set(findfile(savefile2)) then restore, savefile2 else begin
  ...
  ; Don't plot series that previously showed up as no detections. 
  keep[where( moon eq 601 and order eq 5 )] = 0  
  keep[where( moon eq 602 and order eq 4 )] = 0  
  keep[where( moon eq 610 and order eq 4 )] = 0  
  keep[where( moon eq 611 and order eq 4 )] = 0  
  keep[where( moon eq 616 and order eq 3 )] = 0  
  keep[where( moon eq 617 and order eq 3 )] = 0  
  keep[where( moon eq 615 and order eq 2 )] = 0  
...
keep[foo] = 0
;foo = where( _ring_rads gt 136766 )
;keep[foo] = 0
...
for k=0,nmoons-1 do begin
  ...
  if keyword_set(bending) then b=1 else b=0
    ...
    if count gt 0 then begin
     ...
     orderlist = orderlist[uniq(orderlist)]
     case moonlist[k] of
       601: orderlist = vec_remove( orderlist, where(orderlist eq 5) )
       602: orderlist = vec_remove( orderlist, where(orderlist eq 4) )
       610: orderlist = vec_remove( orderlist, where(orderlist eq 4) )
       611: orderlist = vec_remove( orderlist, where(orderlist eq 4) )
       615: orderlist = vec_remove( orderlist, where(orderlist eq 2) )
       616: orderlist = vec_remove( orderlist, where(orderlist eq 3) )
       617: orderlist = vec_remove( orderlist, where(orderlist eq 3) )
       else: a = 5
     endcase 
     norder = n_elements(orderlist)
     for o=0,norder-1 do begin
      ...
      case ring_rads_legend[foo[foo1[0]]] of
        'Pan 92:93': align = 0.9
        'Pr 3:1': align = 0
        'Pr 6:3': align = 0
        'Hy 1:0': align = 0
      ...
      endcase 
      dx = 0.012*( !x.crange[1] - !x.crange[0] )
      if align eq 1 then dx = -dx
      if ring_rads_legend[foo[foo1[ll]]] eq 'En 3:1' and $
                                                     suff eq '_a1' then begin
      endif else begin
        xyouts, tkm(ring_rads[foo[foo1[ll]]])+dx, torq[ll]-.2*ia, charsize=1.5,$
                ring_rads_legend[foo[foo1[ll]]], align=align, co=clr[0]
print, ring_rads_legend[foo[foo1[ll]]]
      endelse
      for m=0,nm-1 do begin
...
xyouts, ints1, replicate(-11.5,n_elements(ints1)), intnames, align=.5

if keyword_set(dolzr) then clzr

if keyword_set(cycle) and not keyword_set(bending) then goto, cyclenext
