pro run_wavelet, _rr, _vv, _wave, wavenum, runits=runits, minlam=minlam, $
        ...
        scale=scale, ytv=ytv, yticki=_yticki, phasepanel=phasepanel, pkt=pkt, $
        ...
        put_yma1=put_yma1, put_yma2=put_yma2, put_yma5=put_yma5, debug=debug, $
        vims=vims, niceplot=niceplot, ring_rads_keep=ring_rads_keep, $
        ring_rads_index=ring_rads_index, ring_rads_maxlam=ring_rads_maxlam, $
        ring_rads_sigma_manual=ring_rads_sigma_manual, xticki=xticki, $
        xticks=xticks, keepplotaxis=keepplotaxis
...
if not keyword_set(noplot) then begin
  ...
  if not keyword_set(noscan) then begin
    ...
    if keyword_set(_yticki) then yticki=_yticki else begin
      if keyword_set(xr) and not keyword_set(keepplotaxis) then begin
        foo = where( rr gt xr[0] and rr lt xr[1], count )
        ...
        maxy = max(vv)
      endelse
      exp = alog10( maxy-miny )
      if exp lt 0 then exp=floor(exp) else exp=ceil(exp)
      yticki = floor(( maxy - miny )/10.^exp) * 10.^exp
      if ceil(miny/yticki) eq floor(maxy/yticki) then yticki=yticki/2
    endelse
    noyti:
    if yticki gt 0 then begin
      yticks = 0
      ytv = 0
    endif else begin
      yticki = 0
      yticks = 2
    endelse 
    plot_nosci, tkm( rr, thoukm=thoukm, phi=phi ), vv, yma=yma1, /xs, /ys, $
                xticki=xticki, xticks=xticks, $
                yticks=yticks, ytickv=ytv, yticki=yticki, $
                xma=xma, xtickn=replicate(' ',20), ytit=ytit, tit=tit, $
  ...
  if keyword_set(ring_rads) and keyword_set(ring_rads_legend) then begin
    if keyword_set(niceplot) then begin
      if keyword_set(xr) then begin
        foo = where( tkm(ring_rads) lt xr[0] or tkm(ring_rads) gt xr[1], count )
      endif else begin
        foo = where( ring_rads lt min(_rr) or ring_rads gt max(_rr), count )
      endelse 
      if count gt 0 then begin
        if keyword_set(ring_rads_keep) then begin
          foo2 = where( ring_rads_keep[foo] eq 1, count2 )
          if count2 eq count then goto, noneleft
          if count2 gt 0 then begin
            foo = vec_remove( foo, foo2 )
            count = count - count2
          endif 
        endif
        if count eq n_elements(ring_rads) then begin
          ring_rads = 0
          ring_rads_legend = ''
          ring_rads_index = ''
          ring_rads_maxlam = 0
          ring_rads_keep = 0
          ring_rads_sigma_manual = 0
        endif else begin
          ring_rads = vec_remove( ring_rads, foo )
          ring_rads_legend = vec_remove( ring_rads_legend, foo )
          if keyword_set(ring_rads_index) then begin
            ring_rads_index = vec_remove( ring_rads_index, foo )
          endif
          if keyword_set(ring_rads_maxlam) then begin
            ring_rads_maxlam = vec_remove( ring_rads_maxlam, foo )
          endif
          if keyword_set(ring_rads_keep) then begin
            ring_rads_keep = vec_remove( ring_rads_keep, foo )
          endif
          if keyword_set(ring_rads_sigma_manual) then begin
            ring_rads_sigma_manual = vec_remove( ring_rads_sigma_manual, foo )
          endif
        endelse
      endif 
      noneleft:
