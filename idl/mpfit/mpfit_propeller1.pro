...
;   MPFIT_PROPELLER
;
; Created by M. Tiscareno, 20 June 2007, by modifying MPFIT2DPEAK
;
; AUTHOR:
...
forward_function mpfit, mpfitfun, mpfit2dpeak, mpfit2dpeak_propeller, $
  mpfit2dpeak_uprop
...
;      A(6)   Peak separation (x)
;      A(7)   Peak separation (y)
;function mpfit2dpeak_propeller, x, y, p, dp, tilt=tilt, symmetric=sym, _extra=extra

function mpfit_propeller1, z, a, x, y, estimates=est, tilt=tilt, $
                      ...
                      nfree=nfree, dof=dof, covar=covar, $
                      ...
                      propeller=propeller, positive=pos
  ...
  if n_elements(est) EQ 0 then begin
      ...
      est = [av, peakz, width, width, centx, centy, 0, 0]
      guess = 1
  endif
  ...
  a = mpfit2dfun(fun, xx, yy, z, 0, p0(0:np-1), weights=weights, $
                 ...
                 functargs=fargs, _EXTRA=extra, covar=covar );, autoderiv=1)
